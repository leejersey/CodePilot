  ...
  return (
    <div>
      ...
      <div className="h-72 overflow-hidden rounded-xl border border-slate-200 dark:border-white/10">
        <Editor value={value} onChange={(next) => onChange(next || "")} language={language === "cpp" ? "cpp" : language} theme={theme === "light" ? "vs" : "vs-dark"} options={{ minimap: { enabled: false }, fontSize: 13, automaticLayout: true, scrollBeyondLastLine: false }} />
      </div>
    </div>
  );
}
